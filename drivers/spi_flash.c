...
static const struct rt_spi_ops sf_bus_ops =
{
    rt_spi_configure,
};
...
{
    //struct rt_spi_device sf_device;
	struct rt_spi_bus sf_bus;
	//struct rt_spi_configuration config;
	
	//config.data_width = SPI_DataSize_8b;
	//config.max_hz = ;
	//config.mode = SPI_Mode_Master;

    //sf_device.bus.ops=&sf_bus_ops;
    //sf_bus.ops=&sf_bus_ops;
	//sf_device.config = config;
	//sf_bus.owner = &sf_device;
	...
	*/
	...
	rt_spi_bus_register(&sf_bus, "SPI BUS Flash", &sf_bus_ops);
